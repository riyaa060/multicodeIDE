import Editor2 from 'react-monaco-editor';
  ...
  const runProject = () => {
    ...
    fetch("https://emkc.org/api/v2/piston/execute", {
      ...
      body: JSON.stringify({
        ...
        version: String(data.version), // Make sure it's a string
        files: [
          {
            filename: data.name + (
      ...
      .then(data => {
        ...
        setError(data.run.code === 1); // `data.run.code === 1` is truthy if there's an error
      })
      .catch((err) => {
      ...
      <div className="flex items-center justify-between" style={{ height: 'calc(100vh - 90px)' }}>
        <div className="left w-[60%] h-full">
          <Editor2
            ...
            height="100%"
            width="100%"
            language={data?.projLanguage || "python"} // Dynamically set language based on data
        ...
        <div className="right p-[15px] w-[50%] h-full bg-[#27272a]">
          ...
          <pre className={`w-full h-[75vh] ${error ? "text-red-500" : ""}`} style={{ whiteSpace: "pre-wrap" }}>
            {output}
          </pre>
        </div>
      </div>
